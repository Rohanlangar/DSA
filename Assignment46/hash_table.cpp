...
int main() {
    ...
    while (true) {
        ...
        if (choice_rrl == 1) {
            ...
            cin >> n_rrl;
            if (n_rrl <= 0) cout << "Invalid size\n";
            else {
                ht_rrl = HashTable_rrl(n_rrl);
                created_rrl = true;
                cout << "Hash table of size " << n_rrl << " created\n";
            }
        } else if (choice_rrl == 2) {
            if (!created_rrl) { cout << "Create table first\n"; continue; }
            int key_rrl; cout << "Enter key to insert: "; cin >> key_rrl;
            if (ht_rrl.insert_rrl(key_rrl)) cout << "Inserted " << key_rrl << "\n";
            else cout << "Insertion failed (table full or duplicate)\n";
        } else if (choice_rrl == 3) {
            if (!created_rrl) { cout << "Create table first\n"; continue; }
            int key_rrl; cout << "Enter key to search: "; cin >> key_rrl;
            ...
            if (!created_rrl) { cout << "Create table first\n"; continue; }
            int key_rrl; cout << "Enter key to delete: "; cin >> key_rrl;
