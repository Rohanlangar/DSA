# Assignment no : 15
### Title : Implement a program to represent binary numbers using doubly linked list and perform operations like 1's complement, 2's complement, and binary addition.

#### Theory :
**Binary Number Representation using Doubly Linked List** allows efficient manipulation of binary numbers of arbitrary length. Each node stores a single bit (0 or 1) with pointers to both previous and next nodes.

**Key Operations:**
1. **1's Complement** - Flip all bits (0→1, 1→0)
2. **2's Complement** - 1's complement + 1 (used for negative number representation)
3. **Binary Addition** - Add two binary numbers with carry propagation
...
```    Bi
naryNumber_rrl onesComplement_rrl() {
...
![alt text](image.png)
https://drive.google.com/file/d/1WuKRkcRGWvlxi4Rnp-6tYNLaGsshxx9T/view?usp=drive_link