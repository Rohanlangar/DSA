...
Graphs can be represented using an **Adjacency Matrix**, which is a 2D array where matrix[i][j] = 1 if an edge exists between vertex i and vertex j, otherwise 0.

**Graph Traversals:**
- **BFS (Breadth First Search)**: Visits nodes level by level using a queue.
- **DFS (Depth First Search)**: Visits nodes by exploring as deep as possible along each branch using a stack.

### Algorithm :
1. Accept the number of vertices and adjacency matrix from the user.
2. For BFS:
   - Use a queue and visited array.
   - Start from the given vertex, explore neighbors, and print nodes in level order.
3. For DFS:
   - Use a stack and visited array.
   - Start from the given vertex, explore nodes as deep as possible, then backtrack.
4. Display menu for user interaction.
