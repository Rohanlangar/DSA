# Assignment no : 16
### Title : Implement a program to represent sets using Generalized Linked List (GLL) with operations for insertion, display, and search of elements and sublists.

#### Theory :
**Generalized Linked List (GLL)** is an advanced data structure that can store both atomic elements and sublists (nested structures). Each node has a flag indicating whether it contains data or points to a sublist.

**Node Structure:**
- **Flag**: 0 for atomic data, 1 for sublist
- **Data**: Actual value (valid when flag = 0)
- **Down**: Pointer to sublist (valid when flag = 1)
- **Next**: Pointer to next element at same level

**Operations:**
1. **Insert Atom** - Add simple data element
2. **Insert Sublist** - Add nested structure
3. **Display** - Recursive traversal with proper formatting
4. **Search** - Recursive search through all levels

**Time Complexity:**
- Insert: O(1) at end of list
- Search: O(n) where n is total elements
- Display: O(n)
...
https://drive.google.com/drive/folders/1QfdfwT49qwADxndCCaGIwM9lk5wngzIS?usp=drive_link