...
A **hash table** stores key–value data for fast access.  
For this program:

- **Key** → Faculty ID  
- **Value** → Faculty Record (Name, Department, Designation)  
- **Hash Function (Divide / MOD Method)**  


---

## **Collision Handling Technique: Linear Probing with Chaining (With Replacement)**

### What it means:
1. Compute the **home slot** using hash  
2. If the home slot is occupied:
 - If the element **in the slot does not belong to this home slot**,  
   → **Replace** it with the new element  
   → Insert the displaced element elsewhere (linear probe)
 - Else  
   → Insert the new element in another slot (linear probe)
3. Maintain a **chain** using a `next[]` array that links all elements belonging to the same home slot  
4. “With Replacement” ensures:  
 **The home slot always contains an element whose hash = that home slot**
...
### **Insert(key, record)**
1. Compute home index = key % size  
2. If home index is empty → insert directly  
3. Else:
 - If existing element doesn’t belong here → replace it  
   - Reinsert displaced element using linear probing  
 - Else  
   - Use linear probing for new element  
4. Update chaining (using next array)

### **Search(key)**
1. Compute home index  
2. Follow the chain using `next[]`  
3. If found → return index  

### **Delete(key)**
1. Search key  
2. Mark slot as deleted  
3. Update chain links  

### **Display()**
Print index, state, next pointer, and record.
