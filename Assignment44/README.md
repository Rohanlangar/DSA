...
**Dijkstra’s algorithm** finds the **shortest path** (minimum distance) from a single source vertex to other vertices in a weighted graph with **non-negative edge weights**.  
**Adjacency List Representation:**  
- Each vertex stores a list of pairs (neighbour, weight).  
...
2. Use a **min-priority queue** (min-heap) to pick the vertex with the smallest distance not yet processed.  
3. For the picked vertex, relax all adjacent edges: if `dist[u] + weight(u,v) < dist[v]`, update `dist[v]` and set parent of `v` to `u`.  
4. Repeat until queue is empty or destination is finalized.  
5. Reconstruct the shortest path using parent array and display the distance.
