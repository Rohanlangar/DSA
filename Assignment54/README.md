...
A **hash table** stores data using a key–value pair for fast searching.  
In this assignment:

- **Key** → student ID  
- **Value** → studeny Record (Name, Dept)  
...
Insert new student using roll number as key.

###  Search Student by Roll Number  
Search by computing hash and probing sequentially.

###  Delete Student  
Mark the record as deleted.

###  Display Table  
Show all student records with their index and state.
...
### **Insert**
1. Compute index using `roll % size`  
2. If slot empty → insert  
3. If slot filled → move forward using linear probing  
4. Insert in next available empty/deleted slot  

### **Search**
1. Compute initial index  
2. Probe until:
   - Roll number matches → return index  
   - An empty slot is encountered → record not found  

### **Delete**
1. Search record  
2. Mark found slot as deleted  
